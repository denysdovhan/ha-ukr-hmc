...
3. **Forecast for all stations**
   ...
   **Sample (Kyiv 33345, 2026‑02‑02)**
   ...
   ```json
   {
     ...
     "I_D": 44,
     "I_N": 69,
     "HM": "хмарно з проясненнями",
     "O_D": "без опадів",
     "O_N": "без опадів",
     "HM_EN": "cloudy with clearings",
     "O_D_EN": "without precipitation",
     "O_N_EN": "without precipitation",
     "WD_N": 36,
     "WD_S": "5-10",
     "WN_N": 36,
     "WN_S": "3-8",
...
4. **Location search and forecast**
   - Search URL: `https://www.meteo.gov.ua/fmi.json?action=getCityList&q={NAME}&limit=10&lang=ua`
   - Search response: GeoJSON features under `cities.features`; locality name and
     region are `properties.name` and `properties.state`, with longitude and
   ...
   without `city` fail even when `latlon` is present. Invalid or unsupported
   ...
   **Hourly schema**
   ...
   - `SmartSymbol`, `SmartSymbolText`, `Weather`: condition code and text
   - `dark`: `1` for night, `0` for day
   - `WindCompass8`, `WindSpeedMS`, `WindGust`: wind direction and speed m/s
   - `Humidity`: relative humidity %
   - `pressure`: pressure hPa
   - `windDirection`: bearing degrees
   ...
   current values and sensors. Physical-station observations are not mixed into
   this mode. Some unavailable numeric forecast fields use `"-"` instead of JSON
   `null`.

   **Website daily-card selection**

   The frontend groups `fulldata` by provider-local date. For each complete day,
   it selects the 03:00 record for the low temperature and night icon, and the
   15:00 record for the high temperature and day icon. This is a deterministic
   selection of published hourly values, not an average or an inferred daily
   condition.

   Home Assistant's daily schema has one condition. The integration therefore
   exposes the 03:00 temperature as `native_templow`, and the 15:00 temperature
   and condition as `native_temperature` and `condition`. A date is omitted when
   either published hour is missing.

5. **Day/night flag + site alerts**
...
### Hydrology posts and radiation stations
...
- Station discovery: parse `METEO_STATIONS` for station id, name, latitude, and longitude.
...
- Location forecast: call `fmi.json?action=getCityWeather` with a non-empty
  label and the configured `latlon`. Validate the echoed point and hourly
...
The integration can query two distinct UkrHMC products without matching station
...
3. **For location forecasts**, call `fmi.json?action=getCityWeather` with a
   display label and `latlon={latitude},{longitude}`. Use the exact point for
   current, hourly, and daily forecast values.
...
Location configuration does not resolve a meteorological station. It stores
the entered label and point, then uses the point forecast consistently for
current values, hourly forecasts, and website-style daily forecasts.
