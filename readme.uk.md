...
> [!NOTE]
> Інтеграція для погодних, радіаційних і гідрологічних даних від
> [Українського гідрометеорологічного центру][ukr-hmc].

> [!IMPORTANT]
> Це незалежний спільнотний проєкт, не пов’язаний з Українським
> гідрометеорологічним центром. Дані можуть відрізнятися від офіційного сайту.
> Завжди дотримуйтеся офіційних попереджень і рекомендацій.

Ця інтеграція додає дані з [meteo.gov.ua][ukr-hmc] до
[Home Assistant][home-assistant] як нативні сутності погоди та сенсори.

## Що надає інтеграція

- **Метеорологічні станції** — виміряні умови, добовий прогноз і прогнози на
  день та ніч для фізичної станції.
- **Погода для місць** — умови на поточну годину, а також погодинний і добовий
  прогнози для точної точки на карті.
- **Радіологічні станції** — потужність експозиційної дози, потужність дози та
  час спостереження у вигляді, опублікованому постачальником.
- **Гідрологічні пости** — рівень води, висота рівня над рівнем моря, добова
  зміна, температура води, гідрологічна ситуація та час спостереження.
- **Кілька джерел** — додайте кілька погодних, радіаційних і гідрологічних
  джерел до одного запису інтеграції.

Налаштовані дані оновлюються кожні 15 хвилин. Інтеграція запитує лише ті
продукти постачальника, які потрібні вибраним джерелам.
...
Найпростіше встановити інтеграцію через [HACS][hacs-url]. Натисніть кнопку
нижче:
...
Інтеграція налаштовується через інтерфейс Home Assistant. Натисніть кнопку
нижче, щоб додати її:
...
2. Натисніть **Додати інтеграцію** та знайдіть **Український
   гідрометеорологічний центр**.
3. Виконайте кроки налаштування.

</details>

Під час налаштування виберіть перше джерело даних. Щоб додати інші джерела,
відкрийте сторінку інтеграції та виберіть потрібну метеорологічну станцію,
погоду для місця, радіологічну станцію або гідрологічний пост.
...
Для погоди за місцем поточна сутність використовує запис постачальника за
поточну годину. Добовий прогноз відповідає поданню meteo.gov.ua: запис на 03:00
надає мінімальну температуру, а запис на 15:00 — максимальну температуру та стан
погоди. Неповні дні пропускаються.

Радіаційні вимірювання та гідрологічні ситуації відображаються так, як їх
опублікував постачальник. Інтеграція не визначає рівень безпеки, не створює
попереджень і не замінює офіційних рекомендацій.

Інтеграція не надає власних дій, тригерів або умов автоматизацій.
Використовуйте її нативні сутності у звичайних автоматизаціях Home Assistant.
